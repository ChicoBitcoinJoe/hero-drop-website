...
export default function Wizard({ roster }) {
  let navigate = useNavigate()
  const characterPath = useParams()['*']
  const character = useCharacter(characterPath)
  ...
  async function loadCharacter() {
    ...
    if(characterPath) {
      ...
      console.log(characterPath, data)
    ...
    <Container>
      ...
      <Grid xs={12}>
        <Stepper activeStep={activeStep} orientation="vertical">
          {/* <Step key="FirstStep"><FirstStep ready={ready} character={character} navigation={navigation} /></Step>
          <Step key="SecondStep"><SecondStep ready={ready} character={character} navigation={navigation} /></Step> 
          <Step key="ThirdStep"><ThirdStep ready={ready} character={character} navigation={navigation} /></Step>
          <Step key="FourthStep"><FourthStep ready={ready} character={character} navigation={navigation} /></Step>
          <Step key="FifthStep"><FifthStep ready={ready} character={character} navigation={navigation} /></Step> */}
        </Stepper>
        {
          activeStep === 5 && (
            <Paper square elevation={0} sx={{ mt: 2, p: 3 }}>
              <Typography>
                All steps completed - If you are satisfied with your character hit the "save" button.
              </Typography>
